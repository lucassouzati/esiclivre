	...
	function resetaSenha($cpfcnpj)
	{
		...
		if (mysql_num_rows($rs)>0)
		{
			...
			if($row['confirmado'])
			{
				...
				$headers .= "Organization: Prefeitura de Itaperuna\r\n";     
				...
				if ($con->query($sql)) 
				{
					...
					if (!sendmail($email,'Redefinição de Senha',$body, $headers))
					{
			...
			else
			{
				...
				if (!sendmail($email,'Solicitação de confirmação de cadastro',$body,$headers))
				{
					$this->erro = "Ocorreu um erro no envio do e-mail. Favor tente mais tarde.";
					return false;
				}
